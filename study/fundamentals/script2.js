...
const ret2 = (birthYear, firstName) =>
  `${firstName}, you have ${65 - (2022 - birthYear)} years until retirement.`;
...
console.log(
  `${jack.firstName} has ${jack.friends.length} friends, and his best friend is ${jack.friends[0]}`
);
...
const bob = {
  ...
  getSummary: function () {
    return `${this.firstName} is a ${this.calcAge()}-year old ${
      ...
      this.hasDriversLicense ? 'has' : "doesn't have" + ' a drivers license'
    }`;
  },
};
...
//continue: If an item doesn't meet the conditions, skip it and continue
for (let i = 0; i < allNum.length; i++) {
  if (allNum[i] % 2 !== 0) continue;
  console.log(allNum[i]);
}

//break: If an item doesn't meet the conditions stop the loop.
for (let i = 0; i < allNum.length; i++) {
  if (allNum[i] === 5) break;
  console.log(allNum[i]);
}
